...
if safe_remove "Test 1" "$test_file" "$content"; then
...
if safe_remove "Test 2" "$test_file" "$content"; then
    expected_content="Initial content
# Another Section
Other content"
    if verify_file_content "$test_file" "$expected_content"; then
...
if safe_remove "Test 3" "$test_file" "$content"; then
    expected_content="Initial content
# Test Title
This is test content 2
This should stay
...
if safe_remove "Test 4" "$test_file" "$content"; then
    expected_content="Initial content
# Other Title
This is other content
More content"
...
if safe_remove "Test 5" "$test_file" "$content"; then
    if [ ! -f "$test_file" ]; then
...
if safe_remove "Test 6" "$test_file" "$content"; then
    expected_content="Initial content
# Title 1
Content for title 1
# Title 2
...
if safe_remove "Test 7" "$test_file" "$content"; then
    expected_content="Initial content"
    if verify_file_content "$test_file" "$expected_content"; then
...
if safe_remove "Test 8" "$test_file" "$content"; then
    expected_content="Initial content
# Title 2
Content for title 2"
    if verify_file_content "$test_file" "$expected_content"; then
...
This is test content line 3 with special chars: !@#$%^&*()"

# Insert content using safe_insert
if safe_insert "Test 9 Insert" "$test_file" "$content"; then
    log_info "Test 9: Content inserted successfully" "test_safe_remove"
    
    # Now remove the content we just added using safe_remove
    if safe_remove "Test 9 Remove" "$test_file" "$content"; then
...
# Add content
content="# Empty File Test
This is content in a previously empty file
Another line of content"

# Insert content
if safe_insert "Test 10 Insert" "$test_file" "$content"; then
    log_info "Test 10: Content inserted into empty file" "test_safe_remove"
    
    # Remove the content
    if safe_remove "Test 10 Remove" "$test_file" "$content"; then
